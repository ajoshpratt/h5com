...
class H5DataLoader(Systems):
    ...
    def HandleMessage(self, msg):
        ...
        if msg.mtype == 'H5_GET_DATASET':
            ...
            self.SendMessage(msg)
        if msg.mtype == 'H5_RETURN_DATASET':
            msg = Msg('print_data', mtype='GTK_PRINT_DATASET', code={ 'data': self.h5[msg.code] })
            self.SendMessage(msg)
        if msg.mtype == 'H5_RETURN_FILE':
            msg = Msg('print_data', mtype='GTK_PRINT_DATA', code={ 'data': self.h5 })
            self.SendMessage(msg)
...
class CellRendererTextWindow(Gtk.Window, Systems):

    def __init__(self, MsgBusInstance, AppStateInstance):
        Systems.__init__(self, MsgBusInstance)
        self.AppState = AppStateInstance
        self.Boxes = self.AppState.Boxes
        self.ActiveBox = self.AppState.getActiveBox
        self.State = self.AppState.getState
        Gtk.Window.__init__(self, title="Thief")
        self.dataset = None
        ...
        #self.grid.attach(self.hbox, 0, 0, 1, 1)

        self.connect('delete_event', Gtk.main_quit)
        msg = Msg('', mtype='H5_RETURN_FILE', code='/')
        self.SendMessage(msg)
        self.show_all()

    def append_parents(self, treelist, names, keylist={}, parent=None):
        try:
            for ikey, key in enumerate(treelist.keys()):
                entry = names.append(parent, [key])
                keylist[ikey] = {'name': key, 'children': {}}
                self.append_parents(treelist[key], names, keylist[ikey]['children'], parent=entry)
        except:
            pass

        return names, keylist


    def createNewListView(self, dataset):
        names = Gtk.TreeStore(str)
        # Here, we're building a tree, which is what we really want for this anyway.
        # So, we want to walk through this...
        names, keylist = self.append_parents(dataset, names, keylist={}, parent=None)
        #print(keylist)

        treeview = Gtk.TreeView(names)
        ...
        self.scrollable_treelist = scrollable_treelist
        #self.grid.attach_next_to(self.scrollable_treelist, self.hbox, Gtk.PositionType.RIGHT, 8, 10)
        self.grid.attach(self.scrollable_treelist, 0, 0, 5, 10)
        #self.hbox.pack_start(button, True, True, 0)
        #treeview.connect('button-press-event', self.left_click_entry)
        treeview.connect('row-activated', self.double_click_entry)
        # We need to handle what happens when an entry is clicked..
        self.names = names
        self.keylist = keylist
        self.show_all()

    def createNewDatasetView(self, dataset, dim=0):
        try:
            shape = dataset.shape
            self.dim = dim
            if shape == 3:
                if dim > shape[2]:
                    ...
                    self.dim = shape[2]
            try:
                names = Gtk.TreeStore(*([str] * (len(dataset.dtype))))

                treeview = Gtk.TreeView(names)
                for i, column_title in enumerate(dataset.dtype.fields.items()):
                    renderer = Gtk.CellRendererText()
                    column = Gtk.TreeViewColumn(column_title[0], renderer, text=i)
                    treeview.append_column(column)
            except:
                names = Gtk.TreeStore(*([str] * (shape[1])))

                treeview = Gtk.TreeView(names)
                for i in range(0, shape[1]):
                    renderer = Gtk.CellRendererText()
                    column = Gtk.TreeViewColumn(i, renderer, text=i)
                    treeview.append_column(column)

            if len(shape) == 2 or len(shape) == 1:
                for row in range(0, shape[0]):
                    a = []
                    for item in dataset[row]:
                        a.append(str(item))
                    names.append(None, a)
            elif len(shape) == 3:
                for row in range(0, shape[0]):
            ...
            scrollable_treelist.add(treeview)
            self.grid.attach_next_to(scrollable_treelist, self.scrollable_treelist, Gtk.PositionType.RIGHT, 10, 9)
            # Add a button to change dimensions...
            #if len(shape) == 3:
            button = Gtk.Button.new_with_label("-")
            button.connect("clicked", self.up_dim)
            self.grid.attach_next_to(button, scrollable_treelist, Gtk.PositionType.BOTTOM, 1,1)
            button_p = Gtk.Button.new_with_label("+")
            button_p.connect("clicked", self.down_dim)
            self.grid.attach_next_to(button_p, button, Gtk.PositionType.RIGHT, 1,1)
            self.show_all()
        except:
            # Probably a group.
            pass

    def up_dim(self, button):
        self.dim += 1
        self.createNewDatasetView(dataset=self.dataset, dim=self.dim)

    def down_dim(self, button):
        if self.dim > 0:
            self.dim -= 1
        self.createNewDatasetView(dataset=self.dataset, dim=self.dim)
    ...
    def returnH5Key(self, treepath):
        name = ''
        keys = self.keylist
        for i in str(treepath).split(':'):
            name += '/' + keys[int(i)]['name']
            keys = keys[int(i)]['children']
        return name


    def double_click_entry(self, treeview, treepath, treeviewcolumn):
        # Treepath indicates the entry in the tree.  We should just have it be handled as a load event, if it's not a dataset...
        ...
        msg = Msg('', mtype='H5_RETURN_DATASET', code=self.returnH5Key(treepath))
        self.SendMessage(msg)
        

    def left_click_entry(self, treeview, event):
    ...
    def HandleMessage(self, msg):
        if msg.mtype == 'GTK_PRINT_DATA':
            self.dataset = msg.code['data']
            self.createNewListView(msg.code['data'])
        if msg.mtype == 'GTK_PRINT_DATASET':
            self.dataset = msg.code['data']
            self.createNewDatasetView(msg.code['data'])
            #self.show_all()
...
gtkwin = CellRendererTextWindow(msgbus, appstate)
...
msgbus.RegisterSystem(gtkwin)
msgbus.LaunchSystems()
# Launch this in a new thread for now.
a = Thread(target=Gtk.main)
a.start()
